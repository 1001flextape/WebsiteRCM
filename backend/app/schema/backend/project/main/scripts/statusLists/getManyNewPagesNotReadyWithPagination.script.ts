...
export default function getManyNewPagesNotReadyWithPagination(d: dependencies) {
  return async (args: input = {}): Promise<returningSuccessObj<findAndCountAll<backendSiteDesignerPage>>> => {
    ...
    const response = await sql.getManyNewPagesNotReadyWithPagination(args).catch(error => d.errorHandler(error, d.loggers))
