...
export default function getManyWithPagination(d: dependencies) {
  ...
  return async (args: input): Promise<returningSuccessObj<findAndCountAll<backendProject>>> => {
    if (!args) {
      args = {
        q: undefined,
        page: 1,
    ...
    let data: findAndCountAll<backendProject> = await db.backendProject.findAndCountAll(search).catch(error => d.errorHandler(error, d.loggers)).catch(error => d.errorHandler(error, d.loggers))
    data.page = page + 1;
