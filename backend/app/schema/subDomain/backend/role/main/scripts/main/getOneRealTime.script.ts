...
export default function getOneRealTime(d: dependencies) {
  return async (args: input): Promise<returningSuccessObj<any>> => {
    ...
    if (doesEntityExist.result) {
      ...
      const permissionIdsForRole = await roleManyPermissionMain.getManyByRoleId({
        roleId: args.id
      })
      const currentPermissionIds = permissionIdsForRole.data.map(p => p.dataValues.permissionId)
      ...
      const setEntity = await sameDoc.set({
        ...
        nonRealTimeProps:{
          id: record.data.dataValues.id,
          currentName: record.data.dataValues.name,
        },
        socketId: args.socketId,
      })
