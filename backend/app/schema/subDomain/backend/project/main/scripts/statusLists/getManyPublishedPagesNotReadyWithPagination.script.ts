...
export default function getManyPublishedPagesNotReadyWithPagination(d: dependencies) {
  return async (args: input = {}): Promise<returningSuccessObj<findAndCountAll<backendSiteDesignerPage>>> => {
    ...
    const response = sql.getManyPublishedPagesNotReadyWithPagination(args).catch(error => d.errorHandler(error, d.loggers))
