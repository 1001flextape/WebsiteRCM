...
export default function getManyDraftedPagesTotalWithPagination(d: dependencies) {
  return async (args: input = {}): Promise<returningSuccessObj<findAndCountAll<backendSiteDesignerPage>>> => {
    ...
    const response = sql.getManyDraftedPagesTotalWithPagination(args).catch(error => d.errorHandler(error, d.loggers))
