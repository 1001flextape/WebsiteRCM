...
export default function getManyPublishedPagesReadyWithPagination(d: dependencies) {
  return async (args: input = {}): Promise<returningSuccessObj<findAndCountAll<backendSiteDesignerPage>>> => {
    ...
    const response = sql.getManyPublishedPagesReadyWithPagination(args).catch(error => d.errorHandler(error, d.loggers))
