...
export default function getManyDraftedPagesNewWithPagination(d: dependencies) {
  return async (args: input = {}): Promise<returningSuccessObj<findAndCountAll<backendSiteDesignerPage>>> => {
    ...
    const response = sql.getManyDraftedPagesNewWithPagination(args).catch(error => d.errorHandler(error, d.loggers))
