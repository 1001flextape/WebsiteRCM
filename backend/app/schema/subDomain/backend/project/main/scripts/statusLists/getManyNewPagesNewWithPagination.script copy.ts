...
export default function getManyNewPagesNewWithPagination(d: dependencies) {
  return async (args: input = {}): Promise<returningSuccessObj<findAndCountAll<backendSiteDesignerPage>>> => {
    ...
    const response = sql.getManyNewPagesNewWithPagination(args).catch(error => d.errorHandler(error, d.loggers))
