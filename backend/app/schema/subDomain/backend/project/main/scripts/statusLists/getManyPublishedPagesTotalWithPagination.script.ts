...
export default function getManyPublishedPagesTotalWithPagination(d: dependencies) {
  return async (args: input = {}): Promise<returningSuccessObj<findAndCountAll<backendSiteDesignerPage>>> => {
    ...
    const response = sql.getManyPublishedPagesTotalWithPagination(args).catch(error => d.errorHandler(error, d.loggers))
