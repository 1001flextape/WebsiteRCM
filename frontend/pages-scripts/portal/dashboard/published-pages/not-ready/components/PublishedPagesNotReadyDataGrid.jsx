...
import { getPublishedPagesNotReadyGraphQL } from '../store/getPublishedPagesNotReady.store';
...
export default function PublishedPagesNotReadyDataGrid() {
  ...
  useEffect(() => {
    getPublishedPagesNotReadyGraphQL({}).then(response => {
      const data = response.data.backendProjectStatusLists_getManyPublishedPagesNotReadyWithPagination || { rows: [] }
