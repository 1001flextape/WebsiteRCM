...
describe("test backendRole.main.js", () => {
  ...
  test("getOneById: backendRoles can add record.", async () => {
    const roleMain = makeBackendRoleMain(d);
    const getRole = await roleMain.getOneById({ id: recordId });
  ...
  test("deleteOne: backendRoles can delete record.", async () => {
    const roleMain = makeBackendRoleMain(d);
    const deletedRole = await roleMain.deleteOne({ id: recordId });
  ...
  test("getPermissionsByRoleId: default role 'media manager inbox only'", async () => {
    const roleMain = makeBackendRoleMain(d);
    const permissions = await roleMain.getPermissionsByRoleId({
