...
import { AvatarGroup, ListItem, ButtonBase, Button } from '@mui/material';
...
import PagesSystemModal from './modal/PagesSystemModal.modal';

// Register the quill-cursors module
Quill.register('modules/cursors', QuillCursors);


function RealTimeLinkSelection({ onTextUpdate, onChangeByUser, ...props }) {
  const quillRef = useRef(null);
  const { idChip, applyTextFieldSelectionBuffer } = useContext(AdminLayoutContext)

  const [isModalOpened, setIsModalOpened] = useState(false)
  const [isPageSystemModalOpened, setIsPageSystemModalOpened] = useState(false)
  ...
  useEffect(() => {
    ...
    if (quillRef.current) {
      ...
      quill.on('text-change', (delta, oldDelta, source) => {
        if (source === 'user') {
          ...
          socket.emit('server-samedoc-linkselection-yjs-update', {
            entity: props.entity,
            name: props.data.name,
            ydoc: updatedYdoc,
          });

          socket.emit('server-samedoc-linkselection-readable-text-update', {
            entity: props.entity,
            name: props.data.name,
      ...
      socket.on('linkselection-yjs-userChangeInput-update', data => {
      ...
      socket.on('samedoc-linkselection-yjs-update', (data) => {
        try {
          if (props.entity === data.entity && props.data.name === data.name) {
        ...
        applyTextFieldSelectionBuffer({
          ...
          quill.on('selection-change', (range, oldRange, source) => {
            if (source === 'user' || source === "api") {
              socket.emit('server-samedoc-linkselection-selection-change', {
                range,
                entity: props.entity,
                name: props.data.name,
              });
          ...
          socket.on('samedoc-linkselection-selection-change', (data) => {
    ...
    return () => {
      ...
      socket.off('linkselection-yjs-userChangeInput-update');
      socket.off('samedoc-linkselection-yjs-update');
      socket.off('samedoc-linkselection-selection-change');
    };
    ...
    <ListItem>
      <div style={{ width: "100%" }}>
        ...
        <Button
          ...
          sx={{ mt: "5px" }}
          onClick={() => {
            setIsPageSystemModalOpened(true)
          }}
        >
          Get Links
        </Button>
      ...
      <PagesSystemModal
        isOpened={isPageSystemModalOpened}
        onClose={() => {
          setIsPageSystemModalOpened(false)
        }}
        setInput={(slug) => {

          const quill = quillRef.current.getEditor();
          // quill.setText(slug); // Clear current content and set new slug

          // Clear the editor content
          quill.setText('');

          // Insert the new slug as if the user did it
          quill.insertText(0, slug, 'user');
          // const socket = initSocket()
          // socket.emit('server-samedoc-linkselection-readable-text-update', {
          //   entity: props.entity,
          //   name: props.data.name,
          //   readableTextValue: slug,
          // });
...
export default RealTimeLinkSelection;
