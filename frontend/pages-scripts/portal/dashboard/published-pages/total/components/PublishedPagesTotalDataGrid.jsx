...
import { getPublishedPagesTotalGraphQL } from '../store/getPublishedPagesTotal.store';
...
export default function PublishedPagesTotalDataGrid() {
  ...
  useEffect(() => {
    getPublishedPagesTotalGraphQL({}).then(response => {
      const data = response.data.backendProjectStatusLists_getManyPublishedPagesTotalWithPagination || { rows: [] }
