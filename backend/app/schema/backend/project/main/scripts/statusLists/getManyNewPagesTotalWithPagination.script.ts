...
export default function getManyNewPagesTotalWithPagination(d: dependencies) {
  return async (args: input = {}): Promise<returningSuccessObj<findAndCountAll<backendSiteDesignerPage>>> => {
    ...
    const response = await sql.getManyNewPagesTotalWithPagination(args).catch(error => d.errorHandler(error, d.loggers))
